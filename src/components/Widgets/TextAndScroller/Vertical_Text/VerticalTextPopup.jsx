...
const VerticalTextPopUp = ({
  ...
  const getFontSize = () => {
    ...
    const baseSize = Math.min(viewportWidth, viewportHeight) * 0.3;
  ...
  const getFontFamily = () => textFont || "Arial, sans-serif";

  const currentSentence = cleanSentences[currentSentenceIndex] || cleanSentences[0];

  // Convert sentence into vertical characters
  const verticalText = currentSentence.split("").map((char, index) => (
      ...
      <div style={{ ...styles.popup, backgroundColor }}>
        ...
        {appName && (
          <div style={styles.titleBar}>
            <h2 style={{ ...styles.appTitle, color: textColor, fontFamily: getFontFamily() }}>
              {appName}
            </h2>
          </div>
        )}
        ...
        <div style={styles.textContainer}>
            ...
            style={{
              ...
              animation: animateTransition
                ? cleanSentences.length > 1
                  ? `fadeInOut ${sentenceDuration}s ease-in-out`
          ...
          {cleanSentences.length > 1 && (
            <div style={styles.indicatorContainer}>
              {cleanSentences.map((_, idx) => (
                <div
                  key={idx}
                  style={{
                    ...styles.indicator,
                    backgroundColor: idx === currentSentenceIndex ? textColor : 'rgba(255,255,255,0.3)',
          ...
          {/* Duration info */}
          {cleanSentences.length > 1 && (
            <div style={{ ...styles.durationInfo, color: textColor, fontFamily: getFontFamily() }}>
        ...
        {tags && tags.length > 0 && (
          <div style={styles.tagsContainer}>
            <div style={{ ...styles.tagsLabel, color: textColor, fontFamily: getFontFamily() }}>Tags:</div>
            <div style={styles.tagsWrapper}>
              {tags.map((tag, index) => (
                <span
                  key={index}
                  style={{ ...styles.tag, color: backgroundColor, backgroundColor: textColor, fontFamily: getFontFamily() }}
                >
                  {tag}
                </span>
              ))}
        ...
        <div style={styles.settingsInfo}>
          <div style={{ ...styles.settingItem, color: textColor, fontFamily: getFontFamily() }}>
...
export default VerticalTextPopUp;
